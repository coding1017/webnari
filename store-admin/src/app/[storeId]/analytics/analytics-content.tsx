...
/** Fill in missing days so charts always show a full timeline, not just a single dot */
function padDailyData(
  days: { date: string; revenue: number; orders: number }[],
  range: string,
) {
  const numDays = range === "7d" ? 7 : range === "90d" ? 90 : range === "all" ? 30 : 30;
...
export default function AnalyticsContent() {
  ...
  useEffect(() => {
    setLoading(true);
    getAnalytics(storeId, range)
      .then((raw) => {
        // Ensure all expected fields exist with safe defaults
        setData({
          range: raw?.range || range,
          revenue: raw?.revenue || { total: 0, previous: 0, change: 0 },
          orders: raw?.orders || { total: 0, previous: 0, change: 0 },
          customers: raw?.customers || { total: 0, new: 0, returning: 0 },
          avgOrderValue: raw?.avgOrderValue || 0,
          revenueByDay: padDailyData(raw?.revenueByDay || [], range),
  ...
  const statusData = data?.statusBreakdown ? Object.entries(data.statusBreakdown).map(([name, value]) => ({ name, value })) : [];
